...
async function deployPriceBulletin(
  ...
  let data = (
    await pricebulletin.populateTransaction.initialize(diamondAddress)
  ).data;
  ...
  if (DEBUG) {
    console.log();
    console.log(
      "PriceBulletin implementation deployed at:",
  ...
  setPublisherPriceBulletin(
    proxy.address,
    "0x2a895CF57ef296d1C63FE082053238D99D749774"
  );
...
if (require.main === module) {
  ...
  if (args.length == 2) {
    ...
    console.error(
      "Usage: node deployPriceBulletin.js <diamondAddr> <chainName>"
    );
    process.exit(1);
  }
}
