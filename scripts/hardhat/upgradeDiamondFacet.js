...
async function upgradeDiamondFacet(
  facetName,
  oldFacetAddress,
  chainName = "localhost"
) {
  if (!facetName || !oldFacetAddress)
    throw "Undefined faceName and/or oldFacetAddress";
  ...
  const oldFacetSelectors = await diamondLoupeFacet.facetFunctionSelectors(
    oldFacetAddress
  );
  ...
  const [adds, replaces, removes] = compareSelectors(
    oldFacetSelectors,
    newFacetSelectors
  );
...
if (require.main === module) {
  ...
  if (args.length !== 3) {
    console.error(
      "Usage: node replaceDiamondFacet.js <facetName> <oldFacetAddr> <chainName>"
    );
    process.exit(1);
  }
