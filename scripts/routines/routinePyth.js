...
async function routinePyth() {
  ...
  const pythAbi = ["function getUpdateFee(bytes[]) view returns (uint)"];
  ...
  if (updateFee.gt(PYTH_UPDATE_PRICE_LIMIT)) {
    ...
    try {
      const tx1 = await pythFacet.storePrice_Pyth(priceUpdateData, {
        value: updateFee,
      });
      await tx1.wait();
      ...
      console.log(
        `${logNewLine(
          "INFO"
        )} Success! Pyth price stored: ${formatedPrice} usd/mxn`
      );
    } catch (error) {
      console.log(
        `${logNewLine("WARN")} Storing Pyth price reverted ERROR: \n${error}`
      );
    }
  }
}
