...
} = require("./routines/routineUpdateAllOracles");
...
async function scheduler() {
  ...
  if (currentDay < 5) {
    ...
    for (const time of scheduledTimes) {
      const scheduledDateTime = currentTimeET.set({
        hour: time.hour,
        minute: time.minute,
      });
    ...
    if (nextScheduledTime === null) {
      ...
      nextScheduledTime = tomorrow.set({
        hour: scheduledTimes[0].hour,
        minute: scheduledTimes[0].minute,
      });
    }
    ...
    if (currentDay == 5) {
      ...
      if (nextScheduledTime === null) {
        ...
        nextScheduledTime = nextMonday.set({
          hour: scheduledTimes[0].hour,
          minute: scheduledTimes[0].minute,
        });
      }
  ...
  console.log(
    `${logNewLine("INFO")} Next scheduler callback in ${durationString} ...`
  );
